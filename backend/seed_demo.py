...
def new_id(prefix: str) -> str:
    return f"{prefix}_{int(datetime.utcnow().timestamp() * 1000)}_{secrets.token_hex(3)}"
...
def create_users(db) -> dict:
    ...
    for (name, role, title, dept, hours, burnout, trend) in EMPLOYEES:
        ...
        user = User(
            ...
            certifications=["BLS", "ACLS"] if "RN" not in title else ["BLS"],
            cert_expiry={"BLS": (datetime.utcnow() + timedelta(days=400)).date().isoformat()},
            preferences={"preferred_shift": "day", "overtime_ok": role != "admin"},
            hired_at=datetime.utcnow() - timedelta(days=400 + (burnout * 5)),
            created_at=datetime.utcnow(),
        )
        db.add(user)
        out[name] = user
    db.commit()
    print(f"  ✓ Created {len(EMPLOYEES)} users (admin: {ADMIN_EMAIL}, employee: {EMP_EMAIL}, pw: {PASSWORD})")
    return out


def create_shifts(db, users: dict) -> list:
    """3 months of shifts. Mix of assigned, open, day/night."""
    shifts = []
    ...
    templates = [
        ("Day Shift — Floor", 3, "Emergency"),
        ("Night Shift — Floor", 2, "Emergency"),
        ("Day ICU Rounds", 2, "ICU"),
        ("Night ICU Rounds", 2, "ICU"),
        ("Day Pediatrics Ward", 2, "Pediatrics"),
        ("Night Pediatrics Ward", 1, "Pediatrics"),
        ("Surgery Block A", 3, "Surgery"),
        ("Surgery Block B", 3, "Surgery"),
        ("Cardiology Clinic", 2, "Cardiology"),
        ("Cardiology Night Telemetry", 1, "Cardiology"),
    ]
    ...
    # Add an "admin coverage" shift occasionally
    admin_templates = [("Admin On-Call", 1, "Administration")]

    for d in range(days):
        day = start + timedelta(days=d)
        for tpl_idx, (title, req, dept) in enumerate(templates + admin_templates):
            # Slight rotation: skip ~10% to create open shifts
            import random
            random.seed(d * 17 + tpl_idx)
            skip = random.random() < 0.10
            start_hour = 7 if "Day" in title or "Clinic" in title or "Rounds" in title else 19
            sh = Shift(
                id=new_id("s"),
                org_id=ORG_ID,
                department=dept,
                ...
                certifications=["BLS", "ACLS"] if dept in ("ICU", "Emergency") else ["BLS"],
                pay_differential="night" if start_hour == 19 else "regular",
...
def create_swaps(db, users, shifts):
    """3 pending swaps with realistic detail."""
    pending_shifts = [s for s in shifts if s.status == "active" and s.assigned_staff]
    if not pending_shifts:
        return

    swap_data = [
        ("RN Priya Sharma",     "childcare conflict — need morning shift"),
        ("Dr. Marcus Holloway", "personal appointment — can swap with colleague"),
        ("RN Carlos Mendez",    "family obligation — would trade for weekend"),
    ]
    for who, reason in swap_data:
        # Find a shift owned by this user
        my_shifts = [s for s in pending_shifts if who in [users[n].name for n in users] and s.employee_id == users[who].id]
        if not my_shifts:
            # Fall back to anyone in their dept
            dept = users[who].department
            my_shifts = [s for s in pending_shifts if s.department == dept][:1]
        if not my_shifts:
            continue
        s = my_shifts[0]
        swap = SwapRequest(
            id=new_id("sw"),
            org_id=ORG_ID,
            requester_id=users[who].id,
            requester_shift_id=s.id,
            reason=reason,
            ai_match_score=82.5,
            status="pending",
        ...
        db.add(swap)
    db.commit()
    print(f"  ✓ Created {len(swap_data)} pending swaps")
...
def create_leaves(db, users):
    ...
    approved_leaves = [
        ("Dr. Liam Foster",     "vacation",  -30, -25, "CME conference"),
        ("Dr. Sophia Martinez", "personal",  -10, -8,  "school event"),
        ("RN Ben Thompson",     "vacation",  -45, -38, "annual leave"),
    ]
    ...
    for who, ltype, offset_s, offset_e, reason in pending_leaves + approved_leaves:
        if who not in users:
            continue
        u = users[who]
        sd = datetime.utcnow() + timedelta(days=offset_s)
        ed = datetime.utcnow() + timedelta(days=offset_e)
        ...
        lv = LeaveRequest(
            ...
            approved_at=None if is_pending else datetime.utcnow() - timedelta(days=2),
...
def main():
    ...
    try:
        ...
        print(f"  Employee login: {EMP_EMAIL} / {PASSWORD}")
        print()
        print(f"  • {len(DEPARTMENTS)} departments")
        print(f"  • {len(EMPLOYEES)} employees")
        print(f"  • ~90 days of shifts (assigned + open)")
        print(f"  • 3 pending swap requests")
        print(f"  • 5 pending + 3 approved leave requests")
